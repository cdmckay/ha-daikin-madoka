...
from .feature import Feature, NotImplementedException
from .connection import Connection, ConnectionException, ConnectionStatus
from .features.fanspeed import FanSpeed
from .features.operationmode import OperationMode
from .features.power import PowerState
from .features.setpoint import SetPoint
from .features.temperatures import Temperatures
from .features.clean_filter import CleanFilterIndicator,ResetCleanFilterTimer
...
class Controller:
    ...
    def __init__(
        ...
        reconnect: bool = True,
        ble_device=None,
        ble_device_provider=None,
    ):
        """Inits the controller with the device address.

        Args:
            address (str): MAC address of the device
            adapter (str): Unused; retained for backwards compatibility.
            ble_device (BLEDevice): Pre-resolved device from Home Assistant.
            ble_device_provider: Optional callable returning the freshest
                BLEDevice for this address (used on every reconnect).
        """
        ...
        self.connection = Connection(
            ...
            ble_device_provider=ble_device_provider,
        )
