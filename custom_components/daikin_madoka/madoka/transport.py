...
class Transport:
    ...
    def rebuild_chunk(self, chunk: bytearray):
        ...
        if len(chunk) < 2:
            logger.info(
                f"Chunk received but discarded due to not enough data ({len(chunk)} bytes)"
            )
            return
        ...
        if self.last_id is not None and chunk_id <= self.last_id:
            logger.debug(
                "Chunks of a new message received while rebuilding another message. Discarding previous chunks..."
            )
